...
const MarketTableSpot = () => {
    const { marketData, pagination, isLoading, pagingState, setQueryParams } =
        useMarketData()

    const spotMarketData = useMemo(() => {
        if (!marketData) return []
        return marketData.filter(
            (crypto) =>
                crypto.marketType === 'spot' || crypto.marketType === 'all',
        )
    }, [marketData])
    ...
    return (
        <DataTable
            ...
            data={spotMarketData}
            loading={isLoading}
            noData={spotMarketData.length === 0}
            onSort={handleSort}
            onPaginationChange={handlePaginationChange}
            onPageSizeChange={handlePageSizeChange}
            pagingData={{
                total: pagination?.total || spotMarketData.length,
...
export default MarketTableSpot
