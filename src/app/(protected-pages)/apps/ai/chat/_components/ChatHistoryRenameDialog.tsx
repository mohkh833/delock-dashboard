...
const ChatHistoryRenameDialog = () => {
    ...
    const {
        ...
        defaultValues: { title: renameDialog.title },
        resolver: zodResolver(validationSchema),
    })

    useEffect(() => {
        if (renameDialog.open) {
            reset({ title: renameDialog.title })
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [renameDialog.title, renameDialog.open])

    const handleDialogClose = () => {
        setRenameDialog({ id: '', title: '', open: false })
    }
    ...
    return (
        <Dialog isOpen={renameDialog.open} onClose={handleDialogClose}>
            <h5>Rename chat</h5>
            <div className="mt-8">
