...
import { addMinutes, differenceInMinutes, isAfter, isBefore } from './utils'
...
function ResizableEvent({
    ...
    const { updateEvent, use24HourFormat } = useFullCalendar()
    ...
    const end = useMemo(() => dayjs(event.endDate).toDate(), [event.endDate])
    ...
    const handleResize: ResizeCallback = useCallback(
        (_, direction, ref) => {
            ...
            setResizePreview({
                start: dayjs(newStart).format(
                    use24HourFormat ? 'HH:mm' : 'h:mm a',
                ),
                end: dayjs(newEnd).format(use24HourFormat ? 'HH:mm' : 'h:mm a'),
            })
